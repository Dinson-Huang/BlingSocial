package com.dinson.blingsocial.core
...
import com.dinson.blingsocial.core.adapter.IJsonAdapter
import com.dinson.blingsocial.core.adapter.IRequestAdapter
import com.dinson.blingsocial.core.adapter.impl.DefaultRequestAdapter
import com.dinson.blingsocial.core.common.SocialConstants
import com.dinson.blingsocial.core.exception.SocialError
import com.dinson.blingsocial.core.listener.FunctionListener
...
import com.dinson.blingsocial.core.model.ShareEntityChecker
...
import com.dinson.blingsocial.core.utils.SocialGoUtils
import com.dinson.blingsocial.core.utils.SocialLogUtils
...
object SocialGo {
    ...
    fun doLogin(
        context: Context?,
        @Target.LoginTarget loginTarget: Int,
        listener: OnLoginListener.() -> Unit
    ) {
        if (context != null) {
     ...
     */
    fun doShare(
        context: Context?, @Target.ShareTarget shareTarget: Int,
    ...
    private fun startShare(
        context: Context,
        @Target.ShareTarget shareTarget: Int,
        entity: ShareEntity,
        listener: OnShareListener
    ) {
        // 对象是否完整
        if (!ShareEntityChecker.checkShareValid(entity, shareTarget)) {
            listener.getFunction().onFailure?.invoke(
                SocialError(
                    SocialError.CODE_SHARE_OBJ_VALID,
                    ShareEntityChecker.getErrorMsg()
                )
            )
            return
        }
    ...
    private fun getShareFinishListener(activity: Activity): OnShareListener {
        ...
        listener.onStart { shareTarget, obj ->
            mShareListener?.getFunction()?.onShareStart?.invoke(
                shareTarget,
                obj
            )
        }
    ...
    private fun prepareImageInBackground(context: Context, entity: ShareEntity) {
        ...
        if (!TextUtils.isEmpty(thumbImagePath) && SocialGoUtils.isHttpPath(thumbImagePath)) {
            ...
            if (SocialGoUtils.isExist(file)) {
                ...
                val localPath =
                    SocialGoUtils.mapResId2LocalPath(context, getConfig().getDefImageResId())
    ...
    fun doPay(
        context: Context?,
        params: String,
        @Target.PayTarget payTarget: Int,
        onFunction: OnPayListener.() -> Unit
    ) {
        if (context != null) {
            val function = FunctionListener()
