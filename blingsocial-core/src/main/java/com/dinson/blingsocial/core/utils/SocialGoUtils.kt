package com.dinson.blingsocial.core.utils
...
object SocialGoUtils {
    ...
    private fun isJpgFile(path: String): Boolean {
        return path.toLowerCase(Locale.getDefault()).endsWith(POINT_JPG) || path.toLowerCase(Locale.getDefault()).endsWith(POINT_JPEG)
    }
    ...
    fun isPngFile(path: String): Boolean {
        return path.toLowerCase(Locale.getDefault()).endsWith(POINT_PNG)
    }
